...
Class Hero{
    ...
    function heroFace(){
        ...
        //if(!file_exists($herofd)){
            $body = imagecreatefrompng(''.GP_LOCATION2.'hero/'.$gdir.'/head/'.$size.'/face0.png');
            if($getbeard!=5 && $gender==0){
                $beard = imagecreatefrompng(''.GP_LOCATION2.'hero/'.$gdir.'/head/'.$size.'/beard/beard'.$getbeard.'-'.$color.'.png');
            }
            $eye = imagecreatefrompng(''.GP_LOCATION2.'hero/'.$gdir.'/head/'.$size.'/eye/eye'.$geteye.'.png');
            $eyebrow = imagecreatefrompng(''.GP_LOCATION2.'hero/'.$gdir.'/head/'.$size.'/eyebrow/eyebrow'.$geteyebrow.(($gender==0)?'-'.$color:'').'.png');
            $hair = imagecreatefrompng(''.GP_LOCATION2.'hero/'.$gdir.'/head/'.$size.'/hair/hair'.$gethair.'-'.$color.'.png');
            $ear = imagecreatefrompng(''.GP_LOCATION2.'hero/'.$gdir.'/head/'.$size.'/ear/ear'.$getear.'.png');
            $mouth = imagecreatefrompng(''.GP_LOCATION2.'hero/'.$gdir.'/head/'.$size.'/mouth/mouth'.$getmouth.'.png');
            $nose = imagecreatefrompng(''.GP_LOCATION2.'hero/'.$gdir.'/head/'.$size.'/nose/nose'.$getnose.'.png');
            $face = imagecreatefrompng(''.GP_LOCATION2.'hero/'.$gdir.'/head/'.$size.'/face/face'.$getface.'.png');
