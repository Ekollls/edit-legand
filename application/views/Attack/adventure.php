     ...
     <div style="background-color:#f4efe4;  margin-right: -5px;  560px;  width: 560px;" >                   
<?php
...
if($adv){
	...
	<div  id="content" class="heroAdventure ">
   
<div id="tileDetails"  class="landscape landscape-<?=$adventure['type_of']?>">
    <div style="margin-right:-8px;width: 540px;  "class="adventureStarted detailImage" ></div>
    <div class="clear"></div>
   <?php $error="";
    ...
    $error=punktxuev7;
        ...
        $error="Герой inосстанаinлиinается. Осталось <span class='timer' counting='down' value='".$timer."'>".$generator->getTimeFormat($session->heroD['revivetime'])."</span>.";
                ...
                <div  class="adventureSend">
                    ...
                    <?php }else{ echo '<div>'; } ?>
                    <?php if($heroF->getHeroStatus() == 100){  ?>
                                    <button <?php if($heroF->getHeroStatus() == 100){ echo 'type="submit"'; } ?> value="<?php echo $lang['Hero']['adv05']; ?>" name="s1" id="start" class="textButtonV2 buttonFramed collect  preventAnimation rectangle withText green">
