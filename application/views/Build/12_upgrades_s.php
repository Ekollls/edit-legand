   ...
   // var_dump(    $abdata);die();
    for($i=($session->tribe*10-9),$ii=0;$i<=($session->tribe*10-2);$i++) { 
        ...
        if ( $technology->getTech(($i-(($session->tribe-1)*10))) || $j == 1 ) {
            ...
            $ii++;
            if(count($ABups) && $ABups[0]['tech']=="b".($i-(($session->tribe-1)*10))){$abdata['b'.$j]+=1;}
            echo "<div class=\"research\">
                <div class=\"bigUnitSection\">
                    <a href=\"#\" onclick=\"return Travian.Game.iPopup(".$i.", 1);\">
                        <img class=\"unitSection u".$i."Section\" src=\"".GP_LOCATION2."x.gif\" alt=\"".$technology->getUnitName($i)."\">
                    </a>
                    <a href=\"#\" class=\"zoom\" onclick=\"return Travian.Game.unitZoom(".$i.");\">
                        <img class=\"zoom\" src=\"".GP_LOCATION2."x.gif\" alt=\"Zoom\">
                    </a>
		    </div>
		<div class=\"information\">
            <div class=\"title\">
            <a href=\"#\" onclick=\"return Travian.Game.iPopup(".$i.", 1);\">
            <img class=\"unit u".$i."\" src=\"".GP_LOCATION2."x.gif\" alt=\"".$technology->getUnitName($i)."\"></a>
            <a href=\"#\" onclick=\"return Travian.Game.iPopup(".$i.", 1);\">".$technology->getUnitName($i)."</a>
            <span class=\"level\">".LVL." ".$abdata['b'.$j]."</span>
        </div>";
            if($abdata['b'.$j] != 20) {
                echo "<div class=\"inlineIconList resourceWrapper\">
                    <div class=\"inlineIcon resource\"><i class=\"r1Big\"></i><span class=\"value value\">".${'ab'.$i}[$abdata['b'.$j]+1]['wood']."</span></div>
                    <div class=\"inlineIcon resource\"><i class=\"r2Big\"></i><span class=\"value value\">".${'ab'.$i}[$abdata['b'.$j]+1]['clay']."</span></div>
                    <div class=\"inlineIcon resource\"><i class=\"r3Big\"></i><span class=\"value value\">".${'ab'.$i}[$abdata['b'.$j]+1]['iron']."</span></div>
                    <div class=\"inlineIcon resource\"><i class=\"r4Big\"></i><span class=\"value value\">".${'ab'.$i}[$abdata['b'.$j]+1]['crop']."</span></div>
                </div>
                <br>
                <div class=\"cta\">
                ...
                $totalRes = ${'ab'.$i}[$abdata['b'.$j]+1]['wood'] + ${'ab'.$i}[$abdata['b'.$j]+1]['clay'] + ${'ab'.$i}[$abdata['b'.$j]+1]['iron'] + ${'ab'.$i}[$abdata['b'.$j]+1]['crop'];
				if($session->gold >= 3 && $building->getTypeLevel(17) >= 1 
				&& (${'ab'.$i}[$abdata['b'.$j]+1]['wood'] > $village->awood || ${'ab'.$i}[$abdata['b'.$j]+1]['clay'] > $village->aclay || ${'ab'.$i}[$abdata['b'.$j]+1]['iron'] > $village->airon || ${'ab'.$i}[$abdata['b'.$j]+1]['crop'] > $village->acrop) 
					&& $totalRes <= $village->resourceSUM) {
                    //echo "&nbsp;&nbsp;<button id='button".crc32($i)."' type=\"button\" value=\"npc\" class=\"icon\">&nbsp;<img src=\"".GP_LOCATION2."x.gif\" style=\"margin-top:6px;\" class=\"npc\" alt=\"npc\"></button>";
                    ?>
								...
								<div class="button-content">تعدیل منابع</div></div></button>
            ...
            if (${'ab'.$i}[$abdata['b'.$j]+1]['wood'] > $village->maxstore || ${'ab'.$i}[$abdata['b'.$j]+1]['clay'] > $village->maxstore || ${'ab'.$i}[$abdata['b'.$j]+1]['iron'] > $village->maxstore) {
                ...
                echo "<button type=\"button\" value=\"Upgrade level\" class=\"green\" onclick=\"window.location.href = 'build.php?id=$id&amp;b=$j&amp;c=$session->mchecker'; return false;\">
                    <div class=\"button-container addHoverClick\">
                    <div class=\"button-background\">
                    <div class=\"buttonStart\">
                        <div class=\"buttonEnd\">
...
if(count($ABups) > 0) {
    ...
    foreach($ABups as $black) {
        ...
        echo "<tr><td class=\"desc\"><img class=\"unit u$unit\" src=\"".GP_LOCATION2."x.gif\" alt=\"".$technology->getUnitName($unit)."\" title=\"".$technology->getUnitName($unit)."\" />".$technology->getUnitName($unit)."</td>";
