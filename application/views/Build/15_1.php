...
if(!empty($_REQUEST["demolish"]) && $_REQUEST["c"] == $session->mchecker) {
	...
	if($_REQUEST["type"] != null && !isset($_REQUEST['tozero']) || $_REQUEST['tozero']==0) 	{
		...
		$database->insertQueueBC($infa[0],9,time(),$infa[1],$village->wid);
		header("Location: build.php?id=".$_GET['id']."&s=15");
...
if($village->resarray['f'.$id] >= 10) {
	?>
	<h4 class="round"><?=$lang['Build']['Demolish'];?></h4>
	<p><?=$lang['Build']['Demolish1'];?></p>
	...
	if (!empty($DemolitionProgress)) {
		...
		echo "</select>";
        //echo "<input type=\"checkbox\" name='tozero' value='1'/> <b>التدمير بالكامل  5 <img src=\"".GP_LOCATION2."x.gif\" class=\"gold\"></b>";
        echo "";
        echo " <button type=\"submit\"  id=\"btn_demolish\" class=\"textButtonV1 green build\"> <div class=\"button-container addHoverClick\">
  ...
  <div class=\"textButtonV1 gold \">" . gz4 . "</div></div></button>";
  if(DEMOLISH_FULL>0){
	
  ?>
<div class="demolishNow">
        <button type="button" class="textButtonV1 gold  " coins="<?php echo DEMOLISH_FULL; ?>" id="buttonRIqnOWSTsTeLO"><div class="button-container addHoverClick">
		<div class="button-background">
			<div class="buttonStart">
				<div class="buttonEnd">
					<div class="buttonMiddle"></div>
		...
		<div class="textButtonV1 gold ">  تخریب کامل <img src="img/x.gif" class="goldIcon" alt=""><span class="goldValue"><?php echo DEMOLISH_FULL; ?></span></div></div></button>
