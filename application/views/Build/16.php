    ...
    if (count($enforcetome) > 0) {
        foreach ($enforcetome as $enforce) {
            ...
            if ($enforce['from'] != 0) {
                ...
                if ($tribe)
                    $output .= '<span class="coordinates coordinatesWrapper coordinatesAligned coordinates<?php echo DIRECTION; ?>"><span class="coordinateX">(' . $info['vx'] . '</span><span class="coordinatePipe">|</span><span class="coordinateY">' . $info['vy'] . ')</span></span><span class="clear"></span>';
                if (!$tribe)
                    $tribe = 4; // Monsters
                $output .= '</th>';
                for ($i = 1; $i <= 10; $i++) {
                    $uni = ($tribe - 1) * 10 + $i;
                    $output .= "<td><img src=\"" . GP_LOCATION2 . "x.gif\" class=\"unit u" . $uni . "\" title=\"" . $technology->getUnitName($uni) . "\" alt=\"" . $technology->getUnitName($uni) . "\" /></td>";
                }

                if ($enforce['u11'] != 0) {
                    $output .= "<td><img src=\"" . GP_LOCATION2 . "x.gif\" class=\"unit uhero\" title=\"Hero\" alt=\"Hero\" /></td>";
                }
    ...
    if (count($enforcetoyou) > 0) {
        ...
        foreach ($enforcetoyou as $enforce) {
            ...
            if ($vr && $fr) {
                ...
                for ($i = 1; $i <= 10; $i++) {
                    $uni = ($tribe - 1) * 10 + $i;
                    echo "<td><img src=\"" . GP_LOCATION2 . "x.gif\" class=\"unit u" . $uni . "\" title=\"" . $technology->getUnitName($uni) . "\" alt=\"" . $technology->getUnitName($uni) . "\" /></td>";
                }
                if ($enforce['u11'] != 0) {
                    echo "<td><img src=\"" . GP_LOCATION2 . "x.gif\" class=\"unit uhero\" title=\"Hero\" alt=\"Hero\" /></td>";
                }
...
<td colspan=\"$colspan\">";
            ...
            echo "<tr><th>&nbsp;</th>";
            for ($i = $start; $i <= ($end); $i++) {
                echo "<td><img src=\"" . GP_LOCATION2 . "x.gif\" class=\"unit u$i\" title=\"" . $technology->getUnitName($i) . "\" alt=\"" . $technology->getUnitName($i) . "\" /></td>";
            }
            if ($prisoners['t11'] != 0) {
                echo "<td><img src=\"" . GP_LOCATION2 . "x.gif\" class=\"unit uhero\" title=\"Hero\" alt=\"Hero\" /></td>";
            }
...
<tbody class=\"infos\"><tr><td colspan=\"" . ($colspan + 1) . "\"><div class='sback'><a href='build.php?t=1&id=39&delprisoners=" . $prisoners['id'] . "'>" . PY11 . "</a></div></td></tr>";
            ...
            if (count($enforcetome) > 0) {
                ...
                foreach ($enforcetome as $enforce) {
                  ...
                  <td colspan=\"" . $colspan . "\">";
                    ...
                    for ($i = 1; $i <= 10; $i++) {
                        $uni = ($tribe - 1) * 10 + $i;
                        echo "<td><img src=\"" . GP_LOCATION2 . "x.gif\" class=\"unit u" . $uni . "\" title=\"" . $technology->getUnitName($i) . "\" alt=\"" . $technology->getUnitName($i) . "\" /></td>";
                    }
                    if ($enforce['u11'] != 0) {
                        echo "<td><img src=\"" . GP_LOCATION2 . "x.gif\" class=\"unit uhero\" title=\"Hero\" alt=\"Hero\" /></td>";
                    }
    ...
    <div id="raidList">
        <?php if (!$session->goldclub) {
            ...
            if ($_GET['action'] == 'editsome') {

                include("application/views/goldClub/farmlist_editsome.php");
    ...
    <div id="raidList">
        ...
        if (!$session->goldclub) {
            ...
            <div class="build_desc">
                <p>با کلوپ طلایی میتوانید نیروها را مخفی کنید و در هنگام حمله به دهکده، آنها را به خارج از دهکده بفرستید تا از آسیب در امان بمانند.</p>
                <button type="button" class="gold textButtonV1 gold builder " id="buttonjzlRVAnXm5wIW">
                    <div class="button-container addHoverClick">
                        ...
                        <div class="button-content">کلوپ طلایی</div>
                    </div>
                </button>
            ...
            <div class="options">
                ...
                if ($session->evasion == 1) {
                    ...
                    <input type="checkbox" class="check" name="hideShow"
                        onclick="window.location.href = '?id=39&disable';" checked="checked"> <?= build21 ?>
                    <?php
                } else {
                    ?>
                    <input type="checkbox" class="check" name="hideShow"
                        onclick="window.location.href = '?id=39&enable';"> <?= build21 ?>
                <?*/
                if (isset($_GET['disable'])) {
                    if(isset($_GET['vilidd'])){
                        $database->query("UPDATE `vdata` SET `evasion`=0 WHERE `owner`='".$session->uid."' and `wref` = '" . $_GET['vilidd'] . "'");
                        header("Location: build.php?id=39&t=100");
                        echo "<SCRIPT language='JavaScript'>window.location='build.php?id=39&t=100';</SCRIPT>";
                   
        
                    }else{
                        $database->query("UPDATE `vdata` SET `evasion`=0 WHERE `owner`='".$session->uid."' ");
                        header("Location: build.php?id=39&t=100");
                        echo "<SCRIPT language='JavaScript'>window.location='build.php?id=39&t=100';</SCRIPT>";
                ...
                if (isset($_GET['enable'])) {
                    if(isset($_GET['vilidd'])){
                        $database->query("UPDATE `vdata` SET `evasion`=1 WHERE `owner`='".$session->uid."' and `wref` = '" . $_GET['vilidd'] . "'");
                        header("Location: build.php?id=39&t=100");
                        echo "<SCRIPT language='JavaScript'>window.location='build.php?id=39&t=100';</SCRIPT>";
                    
                    }else{
                        $database->query("UPDATE `vdata` SET `evasion`=1 WHERE `owner`='".$session->uid."'");
                        header("Location: build.php?id=39&t=100");
                        echo "<SCRIPT language='JavaScript'>window.location='build.php?id=39&t=100';</SCRIPT>";
            ...
            <div class="options">
                ...
                عملیات همکانی:<br>
                فعال سازی گریز در همه دهکده ها: <input type="checkbox" class="check" name="hideShow" onclick="window.location.href = '?id=39&enable';" <?php if ($evasion['evasion'] == 1) {
                     echo "checked=\"checked\"";
                 } else {
                     echo "";
                 } ?>>
                <br>
                غیر فعال سازی گریز در همه دهکده ها: <input type="checkbox" class="check" name="hideShow" onclick="window.location.href = '?id=39&&disable';" <?php if ($evasion['evasion'] == 1) {
                     echo "checked=\"checked\"";
                 } else {
                     echo "";
                 } ?>>
                <?php
                 $villages = $database->query("SELECT * FROM `vdata` WHERE `owner`='" . $session->uid . "'");
    
                 // Loop through each village and display the status and toggle option
                 foreach ($villages as $village) {
                     ?>
                     <hr>
                     دهکده <?php echo htmlspecialchars($village['name']); ?>:
                     <br>
                     وضعیت گریز:
                     <?php echo ($village['evasion'] == 1) ? "فعال" : "غیرفعال"; ?>
                     <br><br>
                     
                     <!-- Checkbox to enable/disable evasion for the individual village -->
                     <input type="checkbox" class="check" name="hideShow"
                            onclick="window.location.href = '?id=39&vilidd=<?php echo $village['wref']; ?>&<?php echo ($village['evasion'] == 1) ? 'disable' : 'enable'; ?>';"
                            <?php if ($village['evasion'] == 1) echo 'checked="checked"'; ?>>
                            <?php echo ($village['evasion'] == 0) ? "فعالسازی" : "غیرفعالسازی"; ?>
                 <?php } ?>
