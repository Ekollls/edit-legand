            ...
            <?php $fList = $database->queryFetch('SELECT * FROM farmlist WHERE owner = '.$session->uid.''); 
                if($fList['id']){ 
                    if(($uinfo['ptime'] > time() || $uinfo['protect']==1)){
                        echo "<span class=\"a arrow disabled\" title=\"player Under beginner protectors\">افزودن به فارم لیست</span>";
                    }else{
                    ?>
                    <a href="build.php?id=39&t=99&action=showSlot&lid=1&vid=<?php echo $getd; ?>&sort=distance&direction=asc"><span class="a arrow GoldClub">افزودن به فارم لیست</span></a>
                <?php } }else{ ?>
                    <span class="a arrow disabled" title="Create a list of farms first From Rally Point">افزودن به فارم لیست</span></span>
                <?php } ?>

            </div>
        <?php } ?>
        <!-- iRedux -> Programm this later
        <div class="option"><span class="a arrow needGoldClub" id="raidListButtonNoGoldClub">افزودن به فارم لیست</span></div>
...
<table cellpadding="0" cellspacing="0" id="troop_info" class="rep transparent">
    ...
    <?php if($query != 0){
        foreach($result as $row){
            ...
            $type = $row['ntype'];
            echo "<tr><td>";
            echo "<img src=\"".GP_LOCATION2."x.gif\" class=\"iReport iReport".$row['ntype']."\" title=\"".$topic."\"> ";
