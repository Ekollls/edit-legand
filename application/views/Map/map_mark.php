...
include('GameEngine/Protection.php');
    ...
    if (
        ...
        while ($y1 >= $y0 && $x0 <= $x1) {
            ...
            $war = mysqli_fetch_assoc(mysqli_query($database->connection,"SELECT `sort_type`,`proc` FROM " . TB_PREFIX . "s1_movement WHERE `from` = '" . $_SESSION['wid'] . "' AND `to` = '" . $row['id'] . "' AND proc=0"));
            if ($war['proc'] == 1) {
                $t = GP_LOCATE . "/img/map/borderatwar.gif";
            }
            $pflagsQ = mysqli_query($database->connection,"SELECT `id`,`x`,`y`,`index`,`text`,`dataId` FROM " . TB_PREFIX . "map_marks WHERE x='" . $x0 . "' AND y='" . $y1 . "' AND type = 'player' ORDER BY id ASC LIMIT 1") or die(mysqli_error($database->connection));
            if ($row2 = mysqli_fetch_assoc($pflagsQ)) {
                switch ($row2['index']) {
                    case 0:
