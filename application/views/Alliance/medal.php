<?php
    ...
    foreach ($varmedal as $medal) {
        ...
        if (isset($bonus[$medal['id']])) {
            $profiel = preg_replace("/\[#" . $medal['id'] . "]/is", '<img src="' . $gpack . ''.GP_LOCATION2.'t/' . $medal['img'] . '.gif" border="0" onmouseout="med_closeDescription()" onmousemove="med_mouseMoveHandler(arguments[0],\'<table><tr><td>' . $titel . '<br /><br />'.AL_WEEK.': ' . $medal['week'] . '</td></tr></table>\')">', $profiel, 1);
        } else {
            $profiel = preg_replace("/\[#" . $medal['id'] . "]/is", '<img src="' . $gpack . ''.GP_LOCATION2.'t/' . $medal['img'] . '.gif" border="0" onmouseout="med_closeDescription()" onmousemove="med_mouseMoveHandler(arguments[0],\'<table><tr><td>'.AL_CATEGORY.':</td><td>' . $titel . '</td></tr><tr><td>'.AL_WEEK.':</td><td>' . $medal['week'] . '</td></tr><tr><td>'.AL_RANK.':</td><td>' . $medal['plaats'] . '</td></tr><tr><td>' . $woord . ':</td><td>' . $medal['points'] . '</td></tr></table>\')">', $profiel, 1);
        }
    }
?>