...
<div id="allyInfoForum">
    <ul><h4 class="chartHeadline">
            <?php
$session_alliance = $session->alliance;
$sql_count = "SELECT COUNT(*) as count FROM forum_topic WHERE alliance = '".$session_alliance."' ";
$result = $database->query($sql_count);
$query = $result[0]['count'];
...
if ($query != 0) {
    $q = "SELECT `post`,`id` FROM forum_topic WHERE alliance = '".$session_alliance."' ORDER BY id DESC $limit";
    $Posts = $database->query($q);
  
    foreach ($Posts as $ps) {
        $q2 = "SELECT * FROM forum_post WHERE `topic`= '".$ps['id']."'  ";
