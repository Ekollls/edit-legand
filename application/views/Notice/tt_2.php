    ...
    $prefix = "ndata";
    $limit2 = "and (ntype=4 or ntype=2 or ntype=3 or ntype=0   or ntype=5 or ntype=6 )";
    $sql = $database->query("SELECT * FROM $prefix WHERE uid = " . $session->uid . " and archive=0 " . $limit2 . " ORDER BY time DESC");
    $query = count($sql);
    ...
    if ($page <= 1 && $lastPage <= 1) {
        ...
        $centerPages .= '<a class="number" href="' . $_SERVER['PHP_SELF'] . '?tiid=' . $_GET['tiid'] . '&page=2">2</a>';

    } elseif ($page == 1 && $lastPage == 3) {
        $centerPages .= '<span class="number currentPage">' . $page . '</span> ';
        $centerPages .= '<a class="number" href="' . $_SERVER['PHP_SELF'] . '?tiid=' . $_GET['tiid'] . '&page=2">2</a> ';
        $centerPages .= '<a class="number" href="' . $_SERVER['PHP_SELF'] . '?tiid=' . $_GET['tiid'] . '&page=3">3</a>';

    } elseif ($page == 1) {
        $centerPages .= '<span class="number currentPage">' . $page . '</span> ';
        $centerPages .= '<a class="number" href="' . $_SERVER['PHP_SELF'] . '?tiid=' . $_GET['tiid'] . '&page=' . $add1 . '">' . $add1 . '</a> ';
        $centerPages .= '<a class="number" href="' . $_SERVER['PHP_SELF'] . '?tiid=' . $_GET['tiid'] . '&page=' . $add2 . '">' . $add2 . '</a> ... ';
        $centerPages .= '<a class="number" href="' . $_SERVER['PHP_SELF'] . '?tiid=' . $_GET['tiid'] . '&page=' . $lastPage . '">' . $lastPage . '</a>';

    } else if ($page == $lastPage && $lastPage == 2) {
        $centerPages .= '<a class="number" href="' . $_SERVER['PHP_SELF'] . '?tiid=' . $_GET['tiid'] . '&page=1">1</a> ';
        $centerPages .= '<span class="number currentPage">' . $page . '</span>';
        ...
        $centerPages .= '<a class="number" href="' . $_SERVER['PHP_SELF'] . '?tiid=' . $_GET['tiid'] . '&page=1">1</a> ... ';
        $centerPages .= '<a class="number" href="' . $_SERVER['PHP_SELF'] . '?tiid=' . $_GET['tiid'] . '&page=' . $sub2 . '">' . $sub2 . '</a> ';
        $centerPages .= '<a class="number" href="' . $_SERVER['PHP_SELF'] . '?tiid=' . $_GET['tiid'] . '&page=' . $sub1 . '">' . $sub1 . '</a> ';
        $centerPages .= '<span class="number currentPage">' . $page . '</span>';
    ...
    $sql2 = $database->query("SELECT * FROM $prefix WHERE uid = " . $session->uid . " and archive=0 " . $limit2 . " ORDER BY time DESC " . $limit);
    $paginationDisplay = "";
    ...
    if ($page == "1" && $lastPage == "1") {
        ...
        $paginationDisplay .= '<img alt="' . PREVPAGE . '" src="img/x.gif" class="previous disabled">';
        $paginationDisplay .= $centerPages;
        $paginationDisplay .= '<a class="next" href="' . $_SERVER['PHP_SELF'] . '?tiid=' . $_GET['tiid'] . '&page=2"><img alt="' . NEXTPAGE . '" src="img/x.gif"></a> ';
        $paginationDisplay .= '<a class="last" href="' . $_SERVER['PHP_SELF'] . '?tiid=' . $_GET['tiid'] . '&page=3"><img alt="' . LASTPAGE . '" src="img/x.gif"></a>';

    } elseif ($page != "1" && $page != $lastPage) {
        $paginationDisplay .= '<a class="first" href="' . $_SERVER['PHP_SELF'] . '?tiid=' . $_GET['tiid'] . '&page=1"><img alt="' . FIRSTPAGE . '" src="img/x.gif"></a> ';
        $paginationDisplay .= '<a class="previous" href="' . $_SERVER['PHP_SELF'] . '?tiid=' . $_GET['tiid'] . '&page=' . $previous . '"><img alt="' . PREVPAGE . '" src="img/x.gif"></a>';
        $paginationDisplay .= $centerPages;
        $paginationDisplay .= '<a class="next" href="' . $_SERVER['PHP_SELF'] . '?tiid=' . $_GET['tiid'] . '&page=' . $nextPage . '"><img alt="' . NEXTPAGE . '" src="img/x.gif"></a> ';
        $paginationDisplay .= '<a class="last" href="' . $_SERVER['PHP_SELF'] . '?tiid=' . $_GET['tiid'] . '&page=' . $lastPage . '"><img alt="' . LASTPAGE . '" src="img/x.gif"></a>';
    ...
    if ($query == 0) {
        ...
        foreach ($sql2 as $row  ) {
            ...
            $viewed = $row["viewed"];
            $archive = $row["archive"];
            $type = (isset($_GET['tiid']) && $_GET['tiid'] == 5) ? $archive : $ntype;
            ...
            if ($type == 9) {
                $outputList .= "<img src=\"".GP_LOCATION2."x.gif\" class=\"iReport iReport21\" alt=\"" . $noticeClass[$ntype] . "\" title=\"" . $noticeClass[$ntype] . "\" /> <div>";
            } else {
                $outputList .= "<img src=\"".GP_LOCATION2."x.gif\" class=\"iReport iReport$type\" alt=\"" . $noticeClass[$type] . "\" title=\"" . $noticeClass[$type] . "\" /> <div>";
            }
            ...
            if ($type == 1 || $type == 2 || $type == 5 || $type == 6 || $type == 7) {
                ...
                $outputList .= "<div class=\"reportInfoIcon\"><img title=\"" . $carry . "\" src=\"".GP_LOCATION2."x.gif\" class=\"reportInfo carry " . $style . "\"></div>";
                ...
                if ($dataarray[1] != 'dead') {
                    if ($dataarray[1] != '') {
                        $outputList .= "<div class=\"reportInfoIcon\"><img title=\"" . $name . " (" . $dataarray[3] . "x)\" src=\"".GP_LOCATION2."x.gif\" class=\"reportInfo itemCategory itemCategory_" . $typeArray[$dataarray[1]] . "\"></div>";
                    }
                } else {
                    $outputList .= "<div class=\"reportInfoIcon\"><img src=\"".GP_LOCATION2."x.gif\" class=\"reportInfo adventureDifficulty0\" title=\"" . HS_HERODEAD . "\"></div>";
                }
            }
...
<form method="post" action="berichte.php" name="msg">
    <input type="hidden" name="t" id="t" value="<?php echo $_GET['tiid']; ?>">
