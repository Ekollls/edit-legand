...
return false;
  ...
  function buildingPOP($f,$lvl){
    ...
    function changeTroops($post){
        ...
        $adminlog="Changed troop anmount in village <a href=\"?p=village&did=$id\">$id</a> ,".time()."";
	   ...
	   if($this->CheckPass($pass,$aID)){
        ...
        foreach($needVillage as $village) {
            ...
            foreach($getmovement as $movedata) {
                ...
                $database->insertQueueBC($movedata['ref'],2,$time,($time+$time2));
            ...
            foreach($enforcement as $enforce) {
                ...
                if(!$targettribe) $targettribe =4; // NoBody - Fixed
                //find slowest unit.
                $unitarray = array();
                for($i=1;$i<=10;$i++){
                    if ($enforce['u'.$i]>0){
                ...
                $database->insertQueueBC($reference,2,$time,($time+$time2));
		...
		header('Location: ?p=deletion&uid='.$id.'&msg=ursdel');
		exit();
	}
  ...
  function getUserActive() {
	...
	$time = time() - (604800);
		...
		$q = "SELECT * FROM users where timestamp > $time and username != 'support' order by timestamp desc";
...
include("../application/TdPaneLXP/function.php");