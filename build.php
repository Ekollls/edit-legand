...
$qc="SELECT * FROM movement where `proc` = '0'   and `moveid` = '".$oldmovement[0]['moveid']."' and `from`='".$village->wid."'";
$resultc=$database->query($qc);
//var_dump(count($resultc));die();
	if (count($resultc)==1){
...
header("Location: ".$_SERVER['PHP_SELF']."?id=".$_GET['id']."&t=1");
    exit();
...
<a id="knowledgeBaseButton" class="contentTitleButton buttonFramed withIcon rectangle green" href="https://forumtravian.ir/" target="_blank">&nbsp;</a>
                        </div>
...
?>
  ...
  <style>
    .rightsideinfo {
      position: absolute; /* یا absolute */
      top: 120px; /* فاصله از بالا */
      left: 5px; /* فاصله از راست */
      width: 300px; /* عرض عنصر */
